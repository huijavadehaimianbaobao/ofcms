...
$(function(){
	...
	function init(){
		$('#listData').datagrid({
	        ...
	        columns: [[
	            ...
	            {field: 'idCard', title: '身份证号', width: 65, align: 'center'},
	            {field: 'tel', title: '电话', width: 65, align: 'center'},
	            {field: 'address', title: '地址', width: 150, align: 'center'},
	            {field: 'opt', title: '操作', align: 'center',width: 40,formatter:function (value,row,index){
	...
	function xiangqing(index){
		...
		/* console.log(row) */
			$('#xname').val(row.name);
			$('#xidcard').val(row.idCard);
			$('#xage').val(row.age);
			$('#xsex').val(row.sexText);
			$('#xtel').val(row.tel);
			$('#xaddress').val(row.address);
			$('#xclassroom').val(row.classroom.name);
			$('#xbirthday').val(row.birthday);
			$('#xjoinTime').val(row.joinTime);
			$('#xmajor').val(row.major.name);
			$('#xstudyTime').val(row.studyTime+"个月");
