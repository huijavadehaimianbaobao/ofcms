...
public class MyRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
		...
		if (user != null) {
			List<UsersRoles> roleByUserId = usersRolesDao.getRoleByUserId(user.getId());
			for (UsersRoles usersRoles : roleByUserId) {
				// 获取角色的ID
				List<RolesPermissions> permissionByRolesId = rolesPermissionsDao
						.getPermissionByRolesId(usersRoles.getRoles().getId());
				System.err.println(permissionByRolesId);
				for (RolesPermissions rolesPermissions : permissionByRolesId) {
					// 将获取到的权限信息放入
					setStringPermissions.add(rolesPermissions.getPermissions().getPerCode());
				}
			}
		}
