...
public class ScoreServiceImpl implements ScoreService {
	...
	public String readExcelFile(MultipartFile file, HttpServletRequest request) {
				...
				if(examName.equals("考试名称为空")){return "考试名称为空";}
				if(examDao.hasExamName(examName)==null){return "无此考试名称，请先录入！";}
				List list2 =(List)list.get(1); 	
				String examDay = (String)list2.get(0); 
				if(examDay.equals("日期不能为空")){return "日期不能为空";}
				List<Course> courses = courseDao.getAllCourse();
				if (list != null && !list.isEmpty()) {
					int suc=0; int err=0; int update1=0;
					List <String>init =(List)list.get(2);	
					for(int z=3;z<list.size();z++){   int number=0; int update=0;
						List<String> one =(List<String>)list.get(z);
						String name =one.get(0);
						List<Student> students =studentDao.getNumber(name);
						if(students.size()==0) return name+"：此姓名不存在，导入失败";
						if(students.size()>1) return name+"：此姓名存在多人，导入失败";	
						String className =one.get(1);
						Integer classNumbers =classroomDao.getNumbers(className);
						if(classNumbers==null)return className+"：此班级不存在，导入失败";
						else if(classNumbers>1)	return className+"：此班级名称重复，导入失败";	
						Exam exam =null;
						 exam =examDao.getOneExam(students.get(0).getId(), examName);
						if(exam==null){
							Classroom classroom =new Classroom();; classroom.setName(className);  
							exam= new Exam();    exam.setClassroom(classroom);exam.setStudent(students.get(0));
							exam.setExamDate(examDay);  exam.setName(examName); 
						...
						if(number>0)suc++;else {err++;}; if(update>0)update1++;
					}
